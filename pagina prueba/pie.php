
    <div class="footer-dark" >
        <footer >
            <div class="container">
                <div class="row">
                    <div class="col-sm col-md item">
                        <h3>Derechos de Uso</h3>
                        <p><a href="https://www.iberley.es/temas/derechos-reales-uso-habitacion-60278">Derechos de uso y habitación</a> </p>
                    </div>
                    <div class="col-sm col-md item">
                        <h3>Patrocinadores</h3>
                        <p><a href="https://amsystem.es/">AM System</a></p>
                        <p><a href="https://www.iesvirgendelcarmen.com/ies/index.php">I.E.S Virgen del Carmen</a></p>
                    </div>
                    <div class="col-sm col-md item">
                        <h3>Privacidad</h3>
                        <p><a href="https://ayudaleyprotecciondatos.es/2018/11/13/derecho-privacidad-espana/"> Derecho de la privacidad Española</a></p>
                    </div>
                    <div class="col-sm col-md item">
                        <h3>Autores</h3>
                        <p><a href="https://github.com/adrianruizmartinez"> Adrián Ruiz</a></p>
                        <p><a href="https://github.com/joseangellopez">José Ángel López</a></p>

                    </div>
                </div>
                <p id="piebajo" class="copyright">Company of Pisé © 2020</p>
            </div>

        </footer>
    </div>
